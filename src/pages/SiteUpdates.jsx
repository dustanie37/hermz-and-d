// SiteUpdates.jsx — the site changelog: dated, newest first.
// 2026-07-16 → 09-05 this was a themed "what the site does" tour; Dustin asked
// for a proper changelog (2026-09-05). Entries stay user-facing — features,
// stats and design, not refactors or data fixes. Add new work under a new
// date at the TOP of ENTRIES (or into today's group if one exists).

const ENTRIES = [
  { date: '2026-09-05', items: [
    { tag: 'Podcast', color: 'cinema', name: 'Run of Show',
      text: 'Every episode page is now the prep doc: six fixed segments, cold open through outro, each with its own notes and questions tagged D, M or both. Film facts and free text live in the Snapshot; ranking history, the scorecard readout and score drift sit inside the Main Discussion.' },
    { tag: 'Podcast', color: 'cinema', name: 'Deep Dives',
      text: 'In the Main Discussion, pick the score categories you want to go long on. Each one opens with both of your score paths across the editions and its own notes — Cinematography on 12 Angry Men is the first.' },
    { tag: 'Podcast', color: 'cinema', name: 'Recording view',
      text: 'A sticky slate strip switches the episode between Edit and Recording. Recording strips the inputs, enlarges the type and hides the workbench — a clean read on the day, tick each note as you cover it. Start over clears everything typed for the episode.' },
    { tag: 'Podcast', color: 'cinema', name: 'Feature library',
      text: 'Recurring features (Craft Draft, Theatre Brain, Scene We’d Teach, Legacy Question) live in an editable library — rename, add or retire them, then pick one or more per episode.' },
  ] },
  ...
  { date: '2026-08-17', items: [
    { tag: 'Podcast', color: 'cinema', name: 'Score Drift',
  ...
  { date: '2026-07-16', items: [
    { tag: 'Films', color: 'film', name: 'Rank Trajectory',
    ...
    { tag: 'Films', color: 'film', name: 'The Podium',
    ...
    { tag: 'Oscars', color: 'gold', name: 'The Upset Board',
  ...
  { date: '2026-07-11', items: [
    ...
    { tag: 'Oscars', color: 'gold', name: 'The Oscar Reveal',
  ...
  { date: '2026-07-10', items: [
    { tag: 'Podcast', color: 'cinema', name: 'The podcast section',
      text: 'The whole podcast section: all 47 episodes with a production dashboard, a per-episode run of show, logistics and chapters, and embedded YouTube with clickable chapters.' },
...
function fmtDate(iso) {
  if (iso === 'earlier') return { big: 'Earlier', small: 'Through June 2026' }
  const [y, m, d] = iso.split('-').map(Number)
  return { big: `${MONTHS[m - 1]} ${d}`, small: String(y) }
}
  ...
  return (
    <div className="min-h-screen bg-night-950">
      ...
      <div className="relative h-[220px] sm:h-[260px] overflow-hidden bg-night-900 flex items-end">
        ...
        <div className="relative max-w-4xl mx-auto px-5 sm:px-8 pb-8 w-full">
          ...
          <p className="text-gray-400 text-base max-w-lg">
            What changed on Hermz &amp; D, newest first.
          </p>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-5 sm:px-8 py-12 space-y-12">
        {ENTRIES.map(group => {
          const d = fmtDate(group.date)
          return (
            <section key={group.date} className="sm:grid sm:grid-cols-[150px_1fr] sm:gap-8">
              <div className="mb-4 sm:mb-0 sm:sticky sm:top-28 sm:self-start">
                <p className="font-display not-italic text-3xl tracking-wide text-white leading-none">{d.big}</p>
                <p className="font-mono text-xs tracking-kicker uppercase text-gold-500 mt-1.5">{d.small}</p>
                <span className="block w-7 h-px bg-gold-500 mt-3" aria-hidden="true" />
              </div>
                ...
                {group.items.map((item, ii) => (
                  <div key={ii}
                    ...
                    <div className="flex items-center gap-3 mb-1.5">
                      <h3 className="font-display not-italic text-xl tracking-wide text-white leading-snug">
                        {item.name}
                      </h3>
